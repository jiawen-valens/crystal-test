...
function showResult() {
  ...
  shareBtn.onclick = function() {
    modalShare.classList.remove('hide');
    shareContent.value = `[水晶人格测试] 我是${info.typeName}，命定水晶：${info.crystalName}！「${info.key}」测你的专属能量水晶：crystal-test.zeabur.app`;
    shareContent.select();
    setTimeout(()=>window.scrollTo(0,0),50);
  };
  closeModalBtn.onclick = function() {
    modalShare.classList.add('hide');
  };
}
